...
class Ripper {
   ...
   */
   parseCardDataFromHtml(html) {
     return new Promise((resolve, reject) => {
       ...
       data.characteristic = this.splitTextList($('.cardDetail > dl:nth-child(9) > dd:nth-child(2)').text());
  ...
  ripCardData(cardRef) {
    ...
    if (!this.isValidPMemoriesUrl(cardRef)) {
      ...
      console.log(`${cardRef} is not a valid prememo URL`)
      return this.lookupCardUrl(cardRef).then((card) => {
        return this.ripCardData(card.cardRef);
      });
    } else {
      console.log(`${cardRef} is a valid URL`)
      return httpAgent
        .request({
          url: normalizeUrl(cardRef)
        })
        .then((res) => {
          return this.parseCardDataFromHtml(res.data, cardRef)
        })
    }
  }
