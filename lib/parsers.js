...
function buildOrder (setAbbr, card) {
  return {
    setAbbr,
    card
  }
 ...
 * 
 * @param {String} query
 * @return {String} setAbbr  - defaults to '*'
 */
function getSetAbbrFromQuery (query) {
  const res = setAbbrRegex.exec(query);
  if (!res) return '*' // default to all sets
  else return res[1];
}
 ...
 *
 * @param {String} query
 * @return {String} card     - defaults to '*'
 */
function getCardFromQuery (query) {
  const res = setlessCardIdRegex.exec(query);
  if (!res) return '*' // default to all cards
  else return res[1];
}
 ...
 * parseQuery
 * 
 * Interprets a search query and decides the cards that the ripper needs to rip in order
 * to satisfy the query.
 * 
 * In other words, we are determining the user's rip target
 * Is it a single card? A whole set? Everything?
 * 
 * @param {String} query
 * @return {Array} targets
 */
function parseQuery (query) {
  return buildOrder(getSetAbbrFromQuery(query), getCardFromQuery(query));
}
 ...
 * normalizeUrl
 * 
 * Ensures that a p-memories url starts with http://p-memories.com
 * 
 * @param {String} url
 * @return {String} normalizedUrl
 */
function normalizeUrl (url) {
  debug(`normalizeUrl() url is ${url}`)
