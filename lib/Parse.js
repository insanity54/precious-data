...
class Parse {
    ...
    getCardFromSearchResults (card, html) {
        ...
        for (const row of rows) {
            ...
            const varieties = PM.getSetAbbrVarieties(officialSetAbbr)
            if (varieties.includes(card.setAbbr)) matchingRow = row;
        }
        

        if (typeof matchingRow === 'undefined') throw new Error('No matching card was found in the html.');
